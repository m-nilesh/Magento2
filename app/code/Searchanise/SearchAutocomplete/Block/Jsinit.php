...
class Jsinit extends Template
{
    ...
    public function getWidgetLink()
    {
        ...
        return preg_replace('/(^https?:\/\/)(.*)(\.js)$/', '//$2', mb_strtolower($search_widgets_link));
    }

    public function getConfiguration()
    {
        if (!$this->apiSeHelper->checkStatusModule() || !$this->apiSeHelper->checkSearchaniseResult(true, null)) {
            return [];
        }
        ...
        $se_service_url = $this->apiSeHelper->getServiceUrl(false);
        ...
        if ($this->apiSeHelper->getIsEnabledInternalJquery()) {
            $params['forceUseExternalJQuery'] = true;
        }
