...
class Queue extends AbstractModel
{
        ...
        if (in_array($action, self::$actionTypes)) {
            ...
            foreach ($data as $d) {
                foreach ($actionStoreIds as $storeId) {
                    if ($action == self::ACT_PREPARE_FULL_IMPORT ||
                        $this->apiSeHelper->getExportStatus($storeId) != ApiSeHelper::EXPORT_STATUS_SUSPENDED
                    ) {
                        $queueData = [
                            'action'    => $action,
                            'data'      => $d,
    ...
    public function clearActions($storeId = null)
    {
        ...
        try {
            if (empty($storeId)) {
                ...
                $whereConditions = [
                    $db->quoteInto('store_id = ?', $storeId)
                ];
                $db->delete($tableName, $whereConditions);
            }
        } catch (\Exception $e) {
            // phpcs:disable Generic.Files.LineLength.TooLong
            $this->loggerHelper->log(__("Unable to clear searchanise_queue with error %1", $e->getMessage()), SeLogger::TYPE_ERROR);
            return false;
        }
    ...
    public function deleteKeys(array $storeIds = [])
    {
        ...
        if (!empty($storeIds)) {
            ...
            try {
                $whereConditions = [
                    $db->quoteInto('store_id IN (?)', $storeIds)
                ];
                $db->delete($tableName, $whereConditions);
            } catch (\Exception $e) {
                // phpcs:disable Generic.Files.LineLength.TooLong
