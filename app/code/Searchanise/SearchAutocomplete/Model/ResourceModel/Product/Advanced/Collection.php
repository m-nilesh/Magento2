...
class Collection extends AdvancedCollection
{
    ...
    public function setOrder($attribute, $dir = self::SORT_ORDER_DESC)
    {
        if ($this->getIsSearchaniseSearchEnabled()) {
            if ($attribute == 'am_relevance') {
                // Workaroud for Amasty Sorting plugin
                $attribute = 'relevance';
            }
    ...
    protected function _renderFiltersBefore()
    {
        if (empty($this->queryText)) {
            $this->queryText = $this->apiSeHelper
                ->getQueryFactory()
                ->get()
                ->getQueryText();
        }
        ...
        if ($processed === true) {
            ...
            try {
                ...
                $temporaryStorageFactory = $this->apiSeHelper->getTemporaryStorageFactory();
                $documentFactory = ObjectManager::getInstance()->get(
    ...
    public function _loadEntities($printQuery = false, $logQuery = false)
    {
        if ($this->searchRequest) {
            ...
            $this->getSelect()
                ...
                ->where('e.entity_id IN (?)', ['in' => $docIds]);
            $this->originalPageSize = $this->_pageSize;
            $this->_pageSize = false;
        }

        $result = parent::_loadEntities($printQuery, $logQuery);

        if ($this->searchRequest) {
            // phpcs:ignore Magento2.Security.LanguageConstruct
            $this->loggerHelper->log($this->getSelect()->__toString(), SeLogger::TYPE_DEBUG);
        }
