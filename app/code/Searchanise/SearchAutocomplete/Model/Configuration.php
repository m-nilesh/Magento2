 ...
 * Configuration class
 *
 * Describe searchanise configuration
 */
class Configuration
{
    ...
    const XML_PATH_REVIEWS_PROVIDER = 'searchanise/searchanise_general/reviews_provider';
    ...
    const XML_PATH_REMOVE_PUB_DIR_FROM_IMAGES_LINKS = 'searchanise/searchanise_general/remove_pub_dir_from_image_links';
    ...
    const XML_PATH_ENABLE_INTERNAL_JQUERY = 'searchanise/searchanise_general/enable_internal_jquery';
    const XML_PATH_EXCLUDED_CMS_PAGES = 'searchanise/searchanise_general/excluded_cms_pages';
    ...
    private static $config_information = [
        ...
        self::XML_PATH_REVIEWS_PROVIDER,
        self::XML_PATH_USE_DIRECT_IMAGES_LINKS,
        self::XML_PATH_REMOVE_PUB_DIR_FROM_IMAGES_LINKS,
        ...
        self::XML_PATH_ENABLE_INTERNAL_JQUERY,
        self::XML_PATH_EXCLUDED_CMS_PAGES,
        'catalog/seo/category_url_suffix',
        'catalog/seo/product_url_suffix',
    ];

    private static $cached_values = [
        self::XML_PATH_API_KEY,
        self::XML_PATH_PRIVATE_KEY,
        self::XML_PATH_PARENT_PRIVATE_KEY,
        self::XML_PATH_CRON_ASYNC_ENABLED,
        self::XML_PATH_AJAX_ASYNC_ENABLED,
        self::XML_PATH_OBJECT_ASYNC_ENABLED,
        self::XML_PATH_SYNC_MODE,
        ...
        if (!empty($xml_path)) {
            if (empty($store_id)) {
                ...
                $value = isset(self::$configCache[$store_id][$xml_path])
                    ...
                    : (isset(self::$configCache[0][$xml_path])
                        ? self::$configCache[0][$xml_path]
    ...
    public function setValue($xml_path, $value, int $store_id = null)
    {
        ...
        if (in_array($xml_path, self::$cached_values)) {
            $this->flush();
        }
    }
    ...
    public function flush($full = false)
    {
        ...
        if ($full) {
            $this->cache->clean([Block::CACHE_TAG]);
    ...
    /**
     * Sets enabled review provider
     *
     * @param string $value   Review provider
     * @param int    $storeId Store identifier
     *
     * @return bool
     */
    public function setReviewsProvider($value, int $storeId = null)
    {
        $this->setValue(self::XML_PATH_REVIEWS_PROVIDER, $value, $storeId);
     ...
     * @return string
     */
    public function getReviewsProvider(int $storeId = null)
    {
        return $this->getValue(self::XML_PATH_REVIEWS_PROVIDER, $storeId);
    }
    ...
    public function getIsRemovePubDirFromImageLinks()
    {
        return $this->getValue(self::XML_PATH_REMOVE_PUB_DIR_FROM_IMAGES_LINKS) == 1;
    }
    ...
    {
        return $this->getValue(self::XML_PATH_ENABLE_INTERNAL_JQUERY) == 1;
    }

    public function getExcludedCmsPages()
    {
        return $this->getValue(self::XML_PATH_EXCLUDED_CMS_PAGES);
    }
    ...
    public function getCurrentEngine()
    {
        ...
        if ($engineResolver) {
            ...
            return  $this->getValue(
                \Magento\CatalogSearch\Model\ResourceModel\EngineProvider::CONFIG_ENGINE_PATH,
                null,
                ''
            );
        }
    ...
    private function getEngineResolver()
    {
        ...
        try {
            // Magento > 2.3.x
            $engineResolver = $objectManager->get('Magento\Framework\Search\EngineResolverInterface');
        } catch (\Exception $e) {
            // Magento 2.1.x, 2.2.
            try {
                $engineResolver = $objectManager->get('Magento\Search\Model\EngineResolver');
            } catch (\Exception $e) {
                return null;
            }
        }
