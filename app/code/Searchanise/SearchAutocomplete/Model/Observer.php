...
class Observer implements ObserverInterface
{
    ...
    {
        $methodName = $this->getMethodName($observer->getEvent());

        try {
            $controller = $observer->getControllerAction();
            $isApiRequest = $controller && $controller->getRequest() && $controller->getRequest()->getControllerModule() == 'Mage_Api';
            $baseUrl = $this->storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB);

            if ($methodName == 'layoutLoadBefore') {
                $allowProcessing = !$isApiRequest && $this->apiSeHelper->checkStatusModule() && $this->apiSeHelper->isHostAvailableForSignup($baseUrl);
            } else {
                $allowProcessing = $this->apiSeHelper->checkStatusModule() && $this->apiSeHelper->checkParentPrivateKey() && $this->apiSeHelper->isHostAvailableForSignup($baseUrl);
            }

            if ($allowProcessing && method_exists($this, $methodName)) {
                $this->{$methodName}($observer->getEvent());

                if ($methodName != 'layoutLoadBefore') {
                    $this->loggerHelper->log(
                        // phpcs:disable Generic.Files.LineLength.TooLong
                        __("Observer::Execute() processed '%1'", $methodName),
                        SeLogger::TYPE_DEBUG
                    );
                }
            }
            ...
            $this->loggerHelper->log(
            // phpcs:disable Generic.Files.LineLength.TooLong
                __("Error: Observer::Execute() for method '%1': [%2] %3", $methodName, $e->getCode(), $e->getMessage()),
                SeLogger::TYPE_ERROR
            );
            $this->notificationHelper->setNotification(
                ...
                __('Searchanise'),
                // phpcs:disable Generic.Files.LineLength.TooLong
                __('Searchanise: Unable to add object data to queue. Error occurs: %1. Please contact Searchanise <a href="mailto:%2">%3</a> technical support', $e->getMessage(), ApiSeHelper::SUPPORT_EMAIL, ApiSeHelper::SUPPORT_EMAIL)
            );
            // Uncomment for debug
            //debug_print_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS);
        } catch (\Error $e) {
            $errorMessage = $e->getMessage() . ' in ' . $e->getFile() . ' file on ' . $e->getLine() . ' line';
            $this->loggerHelper->log($methodName, $errorMessage, SeLogger::TYPE_ERROR);
            $this->notificationHelper->setNotification(
                SeNotification::TYPE_ERROR,
                __('Searchanise'),
                // phpcs:disable Generic.Files.LineLength.TooLong
                __('Searchanise: PHP Error occurs during processing event. Please contact Searchanise <a href="mailto:%1">%2</a> technical support', ApiSeHelper::SUPPORT_EMAIL, ApiSeHelper::SUPPORT_EMAIL)
            );
        }
    }
    ...
    {
        if ($this->apiSeHelper->checkApiKey()) {
    ...
    private function catalogProductSaveBefore(Event $event)
    {
        ...
        if (!empty($product)) {
            $this->queueFactory->create()->addActionDeleteProductFromOldStore($product);
        }
    ...
    private function catalogProductSaveAfter(Event $event)
    {
        ...
        if (!empty($product)) {
            ...
            $this->loggerHelper->log(
                __("Product update hook received for pid: %1", $product->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    private function catalogProductDeleteBefore(Event $event)
    {
        ...
        if ($product) {
            $this->queueFactory->create()->addActionDeleteProduct($product);

            $this->loggerHelper->log(
                __("Product delete hook received for pid: %1", $product->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    private function catalogProductImportBunchDeleteAfter(Event $event)
    {
        if ($products = $event->getBunch()) {
            ...
            foreach ($products as $product) {
                $sku = mb_strtolower($product[ImportProduct::COL_SKU]);
    ...
    private function catalogCategorySaveAfter(Event $event)
    {
        ...
        if ($category && $category->getId()) {
            ...
            $this->loggerHelper->log(
                __("Category update hook received for cid: %1", $category->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    private function catalogCategoryMoveAfter(Event $event)
    {
        ...
        if ($category && $category->getId()) {
            ...
            $this->loggerHelper->log(
                __("Category move hook received for cid: %1", $category->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    private function catalogCategoryDeleteBefore(Event $event)
    {
        ...
        if ($category && $category->getId()) {
            ...
            $this->loggerHelper->log(
                __("Category delete hook received for cid: %1", $category->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    private function modelDeleteBefore(Event $event)
    {
        $object = $event->getData('object');

        if ($object instanceof StoreModel) {
            $this->storeDelete($object);
        }

        if ($object instanceof Website) {
            $this->websiteDelete($object);
        }
     ...
     * @return Observer
     */
    private function storeDelete(StoreModel $store)
    {
        if ($store && $store->getId()) {
            $this->apiSeHelper->deleteKeysForStore($store);

            $this->loggerHelper->log(
                __("Store delete hook recievied. StoreId: %1", $store->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    /**
     * Before delete website
     *
     * @param Website $website
     *
     * @return Observer
     */
    private function websiteDelete(Website $website)
    {
        ...
        $this->loggerHelper->log(
            __("Website delete hook recievied. WebsiteId: %1. StoreIds: %2", $website->getId(), implode(',', array_map(function($store) {
                return $store->getId();
            }, $stores))),
            SeLogger::TYPE_INFO
        );
    ...
    private function storeEdit(Event $event)
    {
        ...
        if ($store && $store->getId()) {
            ...
            if ($isActiveOld != $isActive) {
                if ($this->apiSeHelper->signup($store, false, false) == true) {
                    if ($isActive) {
                        ...
                        $this->loggerHelper->log(
                            __("New search engine for %1 created", $store->getName()),
                            SeLogger::TYPE_INFO
                        );
                    } else {
                        $this->apiSeHelper->sendAddonStatusRequest(ApiSeHelper::SE_ADDON_STATUS_DISABLED, $store);
                        $this->loggerHelper->log(
                            __("Store status set as disabled for %1", $store->getId()),
                            SeLogger::TYPE_INFO
                        );
                    }
                }
            ...
            $this->loggerHelper->log(
                __("Store edit hook recievied. StoreId: %1", $store->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    private function cmsPageDeleteBefore(Event $event)
    {
        ...
        if ($page && $page->getId()) {
            ...
            $this->loggerHelper->log(
                __("Page delete hook received for pid: %1", $page->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    private function cmsPageSaveAfter(Event $event)
    {
        ...
        if ($page && $page->getId()) {
            ...
            $this->loggerHelper->log(
                __("Page update hook received for pid: %1", $page->getId()),
                SeLogger::TYPE_INFO
            );
        }
    ...
    private function addProductToQueue(ProductModel $product)
    {
        ...
        if ($product && $product->getId() && $product->getTypeId() == ProductType::TYPE_SIMPLE) {
