...
class ApiSe extends AbstractHelper
{
    ...
    const EXPORT_STATUS_SUSPENDED  = 'suspended';
    ...
    public static $exportStatusTypes = [
        ...
        self::EXPORT_STATUS_SUSPENDED,
        self::EXPORT_STATUS_SYNC_ERROR,
        self::EXPORT_STATUS_NONE,
    ];

    /**
     * Test hosts list where registration is disallowed
     *
     * @var array
     */
    public static $disallowedHosts = [
        'magento2.docker',
    ];
     ...
     * @return bool
     */
    public function checkApiKey($storeId = null)
    {
    ...
    public function deleteKeysForStore(StoreModel $store, $unsetStoreData = false)
    {
        if ($store) {
            ...
            $this->loggerHelper->log(__("Api & Private keys removed for %1 store", $store->getId()), SeLogger::TYPE_INFO);
    ...
    /**
     * Flushes configuration cache
     *
     * @param bool $full If true, full cache fill be cleared
     *
     * @return string
     */
    public function flushCache($full = false)
    {
        $this->configuration->flush($full);
    }
    ...
    /**
     * Returns true if internal query is enabled
     *
     * @return bool
     */
    public function getIsEnabledInternalJquery()
    {
        if (version_compare($this->getMagentoVersion(), '2.3.0', '<')) {
            return false;
        } else {
            return $this->configuration->getIsEnabledInternalJquery();
        }
    }

    /**
     * Returns CMS pages, excluded from search
     *
     * @param $cached If true, cached value will be used
     *
     * @return array
     */
    public function getExcludedCmsPages($cached = true)
    {
        static $identifiers = null;

        if ($identifiers == null || !$cached) {
            $identifiers = explode(',', $this->configuration->getExcludedCmsPages());
        }
    ...
    public function getIsRenderPageTemplateEnabled()
    {
        if (version_compare($this->getMagentoVersion(), '2.3.0', '<')) {
            ...
            return $this->configuration->getIsRenderPageTemplateEnabled();
        }
    }
    ...
    /**
     * Check if host is available for registration
     *
     * @param string $host
     *
     * @return bool
     */
    public function isHostAvailableForSignup($host)
    {
        $allowed = true;

        foreach (self::$disallowedHosts as $checkHost) {
            if (mb_strstr($host, $checkHost) !== false) {
                $allowed = false;
                break;
            }
        }
    ...
    /**
     * Returns store url for register
     *
     * @param StoreModel $store Current store
     *
     * @return string
     */
    public function getSignupStoreUrl($store = null)
    {
    ...
    public function signup($curStore = null, $showNotification = true, $flSendRequest = true)
    {
        ...
        foreach ($stores as $store) {
            ...
            if ($showNotification == true && empty($isShowed)) {
                ...
                $this->loggerHelper->log("Connecting to Searchanise..", SeLogger::TYPE_INFO);
                $isShowed = true;
            }

            $url = $this->getSignupStoreUrl($store);

            if (!$this->isHostAvailableForSignup($url)) {
                $this->echoConnectProgress("Connecting to {$url} is not allowed. Skipped.", $this->httpResponse);
                // phpcs:disable Generic.Files.LineLength.TooLong
                $this->loggerHelper->log(__("Connecting to %1 host is not allowed. Skipped.", $url), SeLogger::TYPE_INFO);
                continue;
            }
            ...
            if (!empty($response) && $responseKeys = $this->parseResponse($response, true)) {
                ...
                $this->loggerHelper->log(
                    __("Signup new store %1. ApiKey %2, PrivateKey: %3", $store->getName(), $apiKey, $privateKey),
                    SeLogger::TYPE_INFO
                );
            } else {
                $connected = false;
    ...
    public function sendRequest($urlPart, $privateKey, array $data, $onlyHttp = true)
    {
        ...
        if (!empty($privateKey)) {
            ...
            if ($body) {
                $result = $this->parseResponse($body, false, function ($err) use ($privateKey) {
                    if ($err === 'inactive_private_key') {
                        $stores = $this->getStores();

                        foreach ($stores as $store) {
                            if ($this->getPrivateKey($store->getId()) == $privateKey) {
                                $this->setExportStatus(self::EXPORT_STATUS_SUSPENDED, $store->getId());
                                $this->queueFactory->create()->clearActions($store->getId());
                            }
                        }
                    }
                });
    ...
    public function httpRequest(
        ...
        try {
            ...
            $responseHeaders['status_code'] = $response->getStatusCode();
            $responseHeaders['status_message'] = $response->getReasonPhrase();
     ...
     * @param callable $err_callback     Error callback
    ...
    public function parseResponse(
        ...
        try {
            if (trim($jsonData) === 'CLOSED;' || trim($jsonData) === 'CLOSED') {
                ...
                $data = (array) $this->jsonSerializer->unserialize($jsonData);
            }

        } catch (\Exception $e) {
            $this->loggerHelper->log(
                '===== Error ParseResponse: jsonDecode =====',
                $jsonData,
                $e->getMessage()
            );
        ...
        if (empty($data)) {
            ...
            foreach ($data['errors'] as $err) {
                ...
                if (is_callable($err_callback)) {
                    // phpcs:ignore Magento2.Functions.DiscouragedFunction
                    call_user_func($err_callback, $err);
                }
            }
    ...
    /**
     * Parse searchanise state response
     *
     * @param array $response Searchanise response
     *
     * @return array
     */
    public function parseStateResponse($response)
    {
        $data = [];

        if (!empty($response) && !empty($response['variable'])) {
     ...
     * @return bool
     */
    public function startSignup($curStoreId = null)
    {
        if ($this->signup($curStoreId, false) == true) {
            $this->queueImport($curStoreId, false);
            return true;
        }
    ...
    {
        if ($this->checkParentPrivateKey() && !$this->configuration->checkNotificationAsyncCompleted()) {
            ...
            foreach ($stores as $store) {
                ...
                if (!empty($privateKey) &&
                    $this->getExportStatus($store->getId()) == self::EXPORT_STATUS_SENT &&
                    (time() - $this->configuration->getLastRequest() > 10 ||
                        $this->configuration->getLastRequest() - 10 > time() || // It is need if last_request incorrect.
                        $skip_time_check
                    )
                ) {
                    ...
                    $variables = $this->parseStateResponse($response);
     ...
     *
     * @param bool $loginCheck If true, admin login will be checks
     *
     * @return boolean
     */
    public function getIsAdmin($loginCheck = true)
    {
        return $this->appState->getAreaCode() == FrontNameResolver::AREA_CODE &&
            (!$loginCheck || $this->adminSession->isLoggedIn());
    }
    ...
    public function getStores(array $storeIds = [])
    {
        ...
        if (!empty(self::$seStoreIds)) {
            foreach ($stores as $storeId => $store) {
                if (!in_array($storeId, self::$seStoreIds)) {
    ...
    public function getIsSearchaniseSearchEnabled(StoreModel $store = null)
    {
        ...
        // Checks if api key exists
        if (!$this->checkApiKey($store->getId())) {
            return false;
        }

        // Check if page is allowed for search
        if (!in_array(
            $this->httpRequest->getFullActionName(),
            [
    ...
    public function checkSearchaniseResult(
        ...
        if ($this->checkStatusModule($store->getId()) &&
            in_array(
                $exportStatus,
                [self::EXPORT_STATUS_DONE, self::EXPORT_STATUS_SENT, self::EXPORT_STATUS_QUEUED, self::EXPORT_STATUS_PROCESSING]
            ) &&
     ...
     * @param bool $displayErrors      If true, error trace will be added to exceptions
     *
     * @return string Processing result
     * phpcs:disable Generic.Metrics.NestingLevel
     * phpcs:disable Generic.Metrics.NestingLevel.TooHigh
     */
    public function async($flIgnoreProcessing = false, $displayErrors = false)
    {
        ...
        while (!empty($q)) {
            ...
            try {
                ...
                if ($q['action'] == Queue::ACT_PREPARE_FULL_IMPORT) {
                    ...
                    $reviewProvider = isset($info['reviews_provider'])
                        ? (!empty($info['reviews_provider']) ? trim($info['reviews_provider']) : 'N')
                        : 'default'; // No data, use outdated behaviour

                    $this->configuration->setReviewsProvider(
                        $reviewProvider,
                        $store->getId()
                    );
                    ...
                    if ($status == true) {
                        $this->setExportStatus(self::EXPORT_STATUS_SENT, $store->getId());
                    ...
                    if (!empty($dataForSend)) {
                        $dataForSend = $this->jsonSerializer->serialize($dataForSend);
                ...
                $statusError = $e->getMessage() . ' in ' . $e->getFile() . ' file on ' . $e->getLine() . ' line';
    ...
    /**
     * Checks if search result is enabled
     *
     * @param StoreModel $store Store checks
     *
     * @return bool
     */
    public function getIsResultsWidgetEnabled(StoreModel $store = null)
    {
        if (empty($store)) {
            $store = $this->storeManager->getStore();
        }

        return $this->configuration->getIsResultsWidgetEnabled($store->getId());
    }

    /**
     * Returns current review provider
     *
     * @param StoreModel $store
     *
     * @return string
     */
    public function getReviewsProvider(StoreModel $store = null)
    {
        $storeId = $store ? $store->getId() : $this->storeManager->getStore()->getId();

        return $this->configuration->getReviewsProvider($storeId);
    }

    /**
     * Returns true if reviews provider is enabled
     *
     * @param StoreModel $store
     *
     * @return bool
     */
    public function hasReviewsProvider(StoreModel $store = null)
    {
        return $this->getReviewsProvider($store) != 'N';
    }
    ...
    public function checkCurrentEngine(StoreModel $store = null, $showNotification = true)
    {
        ...
        if ($this->getIsResultsWidgetEnabled()) {
            return true;
        }
     ...
     * @return string
     */
    public function stripContentTags($content, $tags = [])
    {
        if (empty($content)) {
            return $content;
        }
    ...
    /**
     * Returns TemporaryStorageFactory
     *
     * @return Magento\Framework\Search\Adapter\Mysql\TemporaryStorageFactory
     */
    public function getTemporaryStorageFactory()
    {
        return $this->temporaryStorageFactory;
    }
