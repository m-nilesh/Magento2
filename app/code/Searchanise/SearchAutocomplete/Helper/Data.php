...
class Data extends AbstractHelper
{
    ...
    private function buildSearchParamsFromRequest($requestType, array $request)
    {
        ...
        if ($requestType == self::TEXT_FIND) {
            $searchRequest['q'] = mb_strtolower(trim($request['query']));
        }
        ...
        if ($requestType == self::TEXT_FIND && empty($searchRequest['q'])) {
    ...
    private function renderSuggestions()
    {
        if ($this->searchaniseRequest &&
            ...
            !class_exists('Magento\Elasticsearch\Model\DataProvider\Base\Suggestions', false)
        ) {
        ...
        if (!empty($facets)) {
            foreach ($facets as $facet) {
                ...
                if (!preg_match("/reviews/", $aggregationName)) {
                    $aggregations[$aggregationName . RequestGenerator::BUCKET_SUFFIX] = $aggregation;
                }
            }
        }

        // Category bucket is required, so adds empty if not exist
        if (empty($aggregations['category' . RequestGenerator::BUCKET_SUFFIX])) {
            $aggregations['category' . RequestGenerator::BUCKET_SUFFIX] = [];
        }
    ...
    public function startEmulation(StoreModel $store = null)
    {
        ...
        if ($store) {
            ...
            $this->initialEnvironmentInfo = $this->appEmulation->startEnvironmentEmulation(
                $store->getId(),
