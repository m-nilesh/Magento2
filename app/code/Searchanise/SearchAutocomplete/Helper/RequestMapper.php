...
class RequestMapper extends AbstractHelper
{
    public function buildQuery(RequestInterface $request)
    {
        ...
        foreach ($sort as $s) {
            $searchaniseQuery['orders'][$s['field']] = mb_strtolower($s['direction']);
    ...
    protected function processQuery($query, array &$result)
    {
        ...
        if (method_exists($query, 'getName') && in_array($query->getName(), ['search', 'partial_search'])) {
    ...
    {
        if (preg_match('/^(.*)_query$/', $name, $matches)) {
            $name = $matches[1];
