...
class Toolbar
{
    ...
    public function afterGetAvailableOrders(
        ...
        if ($this->getIsSearchaniseSearchEnabled()) {
            ...
            $orders['sales_amount'] = __('Bestselling');

            if ($this->apiSeHelper->getReviewsProvider() != 'N') {
                $orders['total_reviews'] = __('Total reviews');
                $orders['reviews_average_score'] = __('Rating');
            }

            $orders['created'] = __('Date');
            $orders['max_discount'] = __('Discount');
