...
abstract class AbstractCommand extends Command
{
    ...
    protected function cleanup($output)
    {
        if ($this->getApiSeHelper()->deleteKeys([], true)) {
            $this->getApiSeHelper()->setParentPrivateKey('');
            $this->configuration->setAutoInstall(false);

            $output->writeln('<info>Searchanise keys were cleanuped succesfully.</info>');
    ...
    protected function createTestPage($title, $storeId = null)
    {
        ...
        $pageFactory = $this->getApiSeHelper()->getPageFactory();
    ...
    protected function processQueue($output, $storeId = null)
    {
        ...
        if ($result == ApiSeHelper::ASYNC_STATUS_OK) {
            ...
            do {
                ...
                sleep(3);
                $info = $this->getApiSeHelper()->sendRequest(
                    '/api/state/get/json',
                    $privateKey,
                    ['status' => '', 'full_import' => ''],
                    true
    ...
    /*protected function createOrder($product_id, $storeId = null)
    {
        ...
        $customerFactory = $this->getApiSeHelper()->getCustomerFactory();
    ...
    public function fullTest($output)
    {
        ...
        // Registration test
        if ($this->cleanup($output) != Cli::RETURN_SUCCESS) {
            return Cli::RETURN_FAILURE;
        }

        if ($this->signup($output) != Cli::RETURN_SUCCESS) {
            return Cli::RETURN_FAILURE;
        }
        ...
        $configurableProduct = $this->createTestProduct(
        ...
        if ($this->processQueue($output, $store->getId()) != Cli::RETURN_SUCCESS) {
        ...
        if ($this->processQueue($output) != Cli::RETURN_SUCCESS) {
